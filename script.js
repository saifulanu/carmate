// Car Maintenance Scheduler - Generated from CSV files

const schedules = {
  "is300": {
    "name": "Lexus IS300",
    "scheduleTypes": {
      "manufacturer": {
        "intervals": [
          8000,
          16000,
          24000,
          32000,
          40000,
          48000,
          56000,
          64000,
          72000,
          80000,
          88000,
          96000,
          104000,
          112000,
          120000,
          128000,
          136000,
          144000,
          152000,
          160000,
          168000,
          176000,
          184000,
          192000,
          200000,
          208000,
          216000,
          224000,
          232000,
          240000
        ],
        "data": {
          "Engine Oil": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Oil Filter": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Cabin Air Filter": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y"
          ],
          "Smart Key Battery": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y"
          ],
          "Brake Fluids": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Engine Air Filter": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Spark Plugs": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Engine Coolant": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y"
          ],
          "Intercooler Coolant": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y"
          ],
          "Fluids ( Brakes, Coolant )": [
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            ""
          ],
          "Axle Shaft Boots": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Body": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Ball Joints / Dust Covers": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Brakes Pads, Calipers, Rotors": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            ""
          ],
          "Brake Lines & Hoses": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Brake Rotor Thickness & Runout": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Drive Belt": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Exhaust Pipes & Mounting": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Fuel Tank Gasket": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Fuel Lines Connection and System Hoses": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Rack-and Pinion Assembly": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Rear Differential Oil": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Steering Linkage & Boots": [
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Transmission For Leakage": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Transmission Fluid Cooler Hoses & Connections": [
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Wiper Blades": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "",
            "",
            "Y",
            "Y"
          ],
          "Tires": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y"
          ],
          "Nuts and Bolts on Chasis": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Car on the road": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            "",
            "Y",
            "Y",
            ""
          ],
          "Oil Maintenance Reminder": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y"
          ],
          "Scheduled Maintenance Reminder": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ]
        }
      },
      "yours": {
        "intervals": [
          3000,
          11000,
          19000,
          27000,
          35000,
          43000,
          51000,
          59000,
          67000,
          75000,
          83000,
          91000,
          99000,
          107000,
          115000,
          123000,
          131000,
          139000,
          147000,
          155000,
          163000,
          171000,
          179000,
          187000,
          195000,
          203000,
          211000,
          219000,
          227000,
          235000
        ],
        "data": {
          "Engine Oil": [],
          "Oil Filter": [],
          "Cabin Air Filter": [],
          "Smart Key Battery": [],
          "Brake Fluids": [],
          "Engine Air Filter": [],
          "Spark Plugs": [],
          "Engine Coolant": [],
          "Intercooler Coolant": [],
          "Fluids ( Brakes, Coolant )": [],
          "Axle Shaft Boots": [],
          "Body": [],
          "Ball Joints / Dust Covers": [],
          "Brakes Pads, Calipers, Rotors": [],
          "Brake Lines & Hoses": [],
          "Brake Rotor Thickness & Runout": [],
          "Drive Belt": [],
          "Exhaust Pipes & Mounting": [],
          "Fuel Tank Gasket": [],
          "Fuel Lines Connection and System Hoses": [],
          "Rack-and Pinion Assembly": [],
          "Rear Differential Oil": [],
          "Steering Linkage & Boots": [],
          "Transmission For Leakage": [],
          "Transmission Fluid Cooler Hoses & Connections": [],
          "Wiper Blades": [],
          "Tires": [],
          "Nuts and Bolts on Chasis": [],
          "Car on the road": [],
          "Oil Maintenance Reminder": [],
          "Scheduled Maintenance Reminder": []
        }
      },
      "recommended": {
        "intervals": [
          3000,
          11000,
          19000,
          27000,
          35000,
          43000,
          51000,
          59000,
          67000,
          75000,
          83000,
          91000,
          99000,
          107000,
          115000,
          123000,
          131000,
          139000,
          147000,
          155000,
          163000,
          171000,
          179000,
          187000,
          195000,
          203000,
          211000,
          219000,
          227000,
          235000
        ],
        "data": {
          "Engine Oil": [],
          "Oil Filter": [],
          "Cabin Air Filter": [],
          "Smart Key Battery": [],
          "Brake Fluids": [],
          "Engine Air Filter": [],
          "Spark Plugs": [],
          "Engine Coolant": [],
          "Intercooler Coolant": [],
          "Fluids ( Brakes, Coolant )": [],
          "Axle Shaft Boots": [],
          "Body": [],
          "Ball Joints / Dust Covers": [],
          "Brakes Pads, Calipers, Rotors": [],
          "Brake Lines & Hoses": [],
          "Brake Rotor Thickness & Runout": [],
          "Drive Belt": [],
          "Exhaust Pipes & Mounting": [],
          "Fuel Tank Gasket": [],
          "Fuel Lines Connection and System Hoses": [],
          "Rack-and Pinion Assembly": [],
          "Rear Differential Oil": [],
          "Steering Linkage & Boots": [],
          "Transmission For Leakage": [],
          "Transmission Fluid Cooler Hoses & Connections": [],
          "Wiper Blades": [],
          "Tires": [],
          "Nuts and Bolts on Chasis": [],
          "Car on the road": [],
          "Oil Maintenance Reminder": [],
          "Scheduled Maintenance Reminder": []
        }
      }
    }
  },
  "ativa": {
    "name": "Perodua Ativa",
    "scheduleTypes": {
      "manufacturer": {
        "intervals": [
          10000,
          20000,
          30000,
          40000,
          50000,
          60000,
          70000,
          80000,
          90000,
          100000,
          110000,
          120000,
          130000,
          140000,
          150000,
          160000,
          170000,
          180000,
          190000
        ],
        "data": {
          "Engine Oil - 0W20 3.5L": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Element S/A Oil Filter": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - Engine Oil": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - CVT": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Element, Air Clearner Filter": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Element, Air Refiner": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            ""
          ],
          "Spark Plugs": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "CVT Fluid - 3L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Brake Fluid - 1L": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Radiator Coolant - 2L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Wiper Blades": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Tires": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ]
        }
      },
      "yours": {
        "intervals": [
          7000,
          17000,
          27000,
          37000,
          47000,
          57000,
          67000,
          77000,
          87000,
          97000,
          107000,
          117000,
          127000,
          137000,
          147000,
          157000,
          167000,
          177000,
          187000
        ],
        "data": {
          "Engine Oil - 0W20 3.5L": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Element S/A Oil Filter": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - Engine Oil": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - CVT": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Element, Air Clearner Filter": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Element, Air Refiner": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            ""
          ],
          "Spark Plugs": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "CVT Fluid - 3L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Brake Fluid - 1L": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Radiator Coolant - 2L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Wiper Blades": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Tires": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ]
        }
      },
      "recommended": {
        "intervals": [
          9000,
          17000,
          25000,
          33000,
          41000,
          49000,
          57000,
          65000,
          73000,
          81000,
          89000,
          97000,
          105000,
          113000,
          121000,
          129000,
          137000,
          145000,
          153000
        ],
        "data": {
          "Engine Oil - 0W20 3.5L": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Element S/A Oil Filter": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - Engine Oil": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Drain Plug Gasket - CVT": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Element, Air Clearner Filter": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Element, Air Refiner": [
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            "",
            "Y",
            ""
          ],
          "Spark Plugs": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "CVT Fluid - 3L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Brake Fluid - 1L": [
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "",
            "",
            "Y",
            "",
            "",
            "",
            "Y",
            ""
          ],
          "Radiator Coolant - 2L": [
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            "",
            ""
          ],
          "Wiper Blades": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ],
          "Tires": [
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y",
            "Y"
          ]
        }
      }
    }
  }
};

// Calculate expected date based on mileage difference
function calculateExpectedDate(currentMileage, nextMileage) {
  const mileageDiff = nextMileage - currentMileage;
  const monthsToAdd = Math.round((mileageDiff / 8000) * 6);
  
  const today = new Date();
  today.setMonth(today.getMonth() + monthsToAdd);
  
  return today.toLocaleDateString('en-MY', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  });
}

// Find next maintenance interval and items
function findNextMaintenance(currentMileage, carKey, scheduleType) {
  const car = schedules[carKey];
  if (!car) return null;
  
  const schedule = car.scheduleTypes[scheduleType];
  if (!schedule) return null;
  
  // Find next interval
  const nextInterval = schedule.intervals.find(interval => interval > currentMileage);
  if (!nextInterval) {
    return {
      found: false,
      message: 'No future scheduled maintenance found.'
    };
  }
  
  // Find items to replace
  const intervalIndex = schedule.intervals.indexOf(nextInterval);
  const replaceItems = [];
  const checkItems = [];
  
  for (const [itemName, values] of Object.entries(schedule.data)) {
    const value = values[intervalIndex];
    if (value && value.trim().toUpperCase() === 'Y') {
      replaceItems.push(itemName);
    }
  }
  
  return {
    found: true,
    nextMileage: nextInterval,
    expectedDate: calculateExpectedDate(currentMileage, nextInterval),
    replaceItems: replaceItems,
    checkItems: checkItems
  };
}

// UI event handlers
document.addEventListener('DOMContentLoaded', function() {
  const calcBtn = document.getElementById('calcBtn');
  const mileageInput = document.getElementById('mileage');
  const carSelect = document.getElementById('carSelect');
  const resultDiv = document.getElementById('result');
  
  calcBtn.addEventListener('click', function() {
    const currentMileage = parseInt(mileageInput.value);
    const carKey = carSelect.value;
    
    // Get selected schedule type
    const scheduleTypeRadios = document.getElementsByName('scheduleType');
    let scheduleType = 'manufacturer';
    for (const radio of scheduleTypeRadios) {
      if (radio.checked) {
        scheduleType = radio.value;
        break;
      }
    }
    
    if (!currentMileage || currentMileage <= 0) {
      alert('Please enter a valid mileage.');
      return;
    }
    
    const result = findNextMaintenance(currentMileage, carKey, scheduleType);
    
    if (!result.found) {
      resultDiv.style.display = 'none';
      alert(result.message);
      return;
    }
    
    // Update UI
    document.getElementById('nextMileage').textContent = result.nextMileage.toLocaleString() + ' km';
    document.getElementById('nextDate').textContent = result.expectedDate;
    
    // Clear previous columns
    const colsContainer = document.querySelector('.cols');
    colsContainer.innerHTML = '';
    
    // Add Replace column
    if (result.replaceItems.length > 0) {
      const replaceCol = document.createElement('div');
      replaceCol.className = 'col';
      replaceCol.innerHTML = '<h3>Replace</h3><ul>' + 
        result.replaceItems.map(item => `<li>${item}</li>`).join('') + 
        '</ul>';
      colsContainer.appendChild(replaceCol);
    }
    
    // Add Check column if needed
    if (result.checkItems.length > 0) {
      const checkCol = document.createElement('div');
      checkCol.className = 'col';
      checkCol.innerHTML = '<h3>Check</h3><ul>' + 
        result.checkItems.map(item => `<li>${item}</li>`).join('') + 
        '</ul>';
      colsContainer.appendChild(checkCol);
    }
    
    resultDiv.style.display = 'block';
  });
});

console.log('Maintenance scheduler loaded. Data for', Object.keys(schedules).length, 'cars.');
